...
def compare_the_detectors_for_each_behaviour_for_each_metric_for_each_classification(df, detectors, behaviours, metrics,
                                                                                     classifications):
    # Selecting behaviour
    for behaviour in behaviours:
        ...
        for metric in metrics:
            ...
            for classification in classifications:
                ...
                generate_metric_plot_for_each_detector(behaviour, detectors, metric, classification,
                                                       df_behaviour_classification)
...
def generate_avg_bill_for_each_behaviour_for_60_weeks(behaviours, type_of_dataset, meterIDs):
    ...
    for behaviour in behaviours:
        ...
        for meterID in meterIDs:
            if behaviour == 'False':
                df_meterID = pd.read_csv(
                    './script_results/' + type_of_dataset + '_training_data/' + str(meterID) + '_0_60.csv')
            else:
                df_meterID = pd.read_csv('./script_results/' + type_of_dataset + '_training_data/' + str(
                    meterID) + '_' + behaviour + '_0_60.csv')
