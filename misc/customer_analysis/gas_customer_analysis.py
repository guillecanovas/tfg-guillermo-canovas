...
3. Generates a csv (gas_results_customers_analysis.csv). Example:
...
Observation: later, gas_results_customers_analysis.csv was manually updated writing (yes/no) according
...
results = "./script_results/gas_results_customers_analysis.csv"
...
def generate_results_customers_analysis(meterID, classification, result_adf, result_kpss, output):
    ...
    df.to_csv(output, header=meterID == 2513, index=False)


if __name__ == '__main__':
    meterIDs = get_158_allocation_1_meter_ids() + get_152_allocation_2_meter_ids() + get_168_allocation_3_meter_ids() + \
                get_170_allocation_4_meter_ids() + get_352_allocation_5_meter_ids()

    count_meter_ids_proportion(meterIDs, "Gas")
    ...
    for meterID in meterIDs:
        classification = get_classification_of_meter_id(meterID, "Gas")
        tsa = TSanalyzer()
