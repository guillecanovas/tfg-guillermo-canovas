...
from:
    ./script_results/<dataset>_detector_comparer_results_new.csv
...
from src import meterIDsElectricity, meterIDsGas, meterIDsSolar
...
def set_and_get_variables(type_of_dataset):
    ...
    if type_of_dataset == "solar_generation" or type_of_dataset == "solar_consumption":
        detector_comparer = pd.read_csv("./script_results/" + type_of_dataset + "_detector_comparer_results_prueba_v2.csv")
...
if __name__ == '__main__':
    """
    args: 
    sys.argv[1]:type (electricity, gas, solar)
    sys.argv[2]:type (generation, consumption, teg) (optional)
    sys.argv[2]:type (teg) (optional)
    """
    type_of_dataset = sys.argv[1]
    evaluate_teg = False

    if(len(sys.argv) == 3):
        if (type_of_dataset == 'electricity' or type_of_dataset == 'gas' or type_of_dataset == 'solar_generation' or type_of_dataset == 'solar_consumption' ):
            if(sys.argv[2] == "teg"): 
                evaluate_teg = True
        else:
            print("Usage: python3 dashboardEDCC21.py <electricity, gas, solar_consumption, solar_generation> <teg>(Optional)")
            sys.exit(1)
    ...
    print("Evaluate TEG: " + str(evaluate_teg))
    ...
    if type_of_dataset == "solar_generation":
        if evaluate_teg == True:
            detectors = ["TEG_Hamming_30" , "TEG_Cosine_30", "TEG_Jaccard_30", "TEG_Dice_30", "TEG_KL_30", "TEG_Jeffreys_30", "TEG_JS_30", 
        ...
        behaviours = ['False', 'Percentile', 'Rating', 'RSA_0.5_3']
        metrics = ['accuracy']
