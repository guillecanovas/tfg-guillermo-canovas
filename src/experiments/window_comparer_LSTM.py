...
def create_sequence(train, test, time_steps):
...
def metrics_to_csv(meterID, detector, attack, time_model_creation, time_model_prediction, n_tp, n_tn, n_fp, n_fn, type_of_dataset):
    ...
    resulting_csv_path = "./script_results/window_size_comparer/LSTM/" + type_of_dataset + "_detector_comparer_results_LSTM_window.csv"
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print("Usage: python3 window_comparer_LSTM.py <electricity|gas|solar_consumption|solar_generation> <window_size>")
        sys.exit(1)

    TYPE_OF_DATASET = sys.argv[1]
    WINDOW_SIZE = int(sys.argv[2])  # Lenght of the "lookback window"
    ...
    name_of_detector = "LSTM_" + str(WINDOW_SIZE)
    ...
    for meterID in meterIDs:
        for attack in tuple_of_attacks:
            ...
            metrics_to_csv(meterID, name_of_detector, attack, time_model_creation, time_model_prediction, n_tp, n_tn, n_fp, n_fn, type_of_dataset)
