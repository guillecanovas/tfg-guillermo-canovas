...
teg module (tegdet library) adapted for the diaspore framework
...
from src.detectors.tegdet.graph_comparison import *

#TEGDetector is the API of the tegdet library: adapted to a subclass of Detector in diaspore
class TEGDetector(Detector):
    """ 
    API 
    """
    #Default values
    ...
    def get_dataset(self,ds_path):
        ...
        data points (Usage), respectively, and return the pandas dataframe "dt"
        """
        df = pd.read_csv(ds_path)
        #Diaspore adaptation: TS->TD, DP->Usage
        df.columns = ['TD','Usage']
        return df
    ...
    def build_model(self, training_dataset):
        ...
        """
        t0 = time()
        #Diaspore adaptation: DP->Usage
        obs = training_dataset['Usage']
    ...
    def predict(self, testing_dataset, model):
        ...
        """
        t0 = time()
        data_points = testing_dataset['Usage']
        ...
        #Diaspore adaptation: the first return-parameter is the tot. number of outliers
        return np.sum(outliers), int(len(testing_dataset.index) / self.__n_obs_per_period), time() - t0
    ...
    def print_results(self, detector, testing_set, perf, cm):
        """
    ...
    def results_to_csv(self, detector, testing_set, perf, cm,results_csv_path):
        """
