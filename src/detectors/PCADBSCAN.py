...
class PCADBSCAN(Detector):
    ...
    def build_model(self, training_dataset):
        ...
        mg.updateMeterID("./datasets/ISSDA-CER/Electricity/doc/customerClassification.csv")
    ...
    def get_training_dataset(self, meterID, type_of_dataset):
        ...
        if os.path.isfile("./script_results/pca_dbscan_training.csv"):
            ...
            print("Generating PCA-DBSCAN training file...")
            mg = dataAnalyzerPCA()
            files = mg.getDataSetFiles("./datasets/ISSDA-CER/Electricity/data/data_all_filtered")
    ...
    def metrics_to_csv(self, meterID, detector, attack, time_model_creation, time_model_prediction, n_tp, n_tn, n_fp, n_fn, type_of_dataset):
        resulting_csv_path = "./script_results/" + type_of_dataset + "_detector_comparer_results_new.csv"
...
class dataAnalyzerPCA:
    ...
    def loadFilteredData(self, files, firstWeek, lastWeek):
        ...
        for file in files[firstWeek:lastWeek]:
            # Read the csv file
            ...
            week_number = file_name.split(' ')[-1].replace('.csv', '')

            # Add a new column with the week
            df_temp['Week'] = week_number
