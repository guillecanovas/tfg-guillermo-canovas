...
ELECTRICITY_DATA_ALL_FILTERED_PATH =        ISSDA_PATH + "Electricity/data/data_all_filtered/"
ELECTRICITY_DATA_ORIGINAL_PATH =            ISSDA_PATH + "Electricity/data/data_original/"
ELECTRICITY_DATA_ORIGINAL_SORTED_PATH =     ELECTRICITY_DATA_ORIGINAL_PATH + "sorted"
ELECTRICITY_DATA_ALL_PATH =                 ISSDA_PATH + "Electricity/data/data_all/" 
ELECTRICITY_CUSTOMER_CLASSIFICATION_PATH =  ISSDA_PATH + "Electricity/doc/customerClassification.csv"

GAS_DATA_ALL_FILTERED_PATH =            ISSDA_PATH + "Gas/data/data_all_filtered/"
GAS_DATA_ORIGINAL_PATH =                ISSDA_PATH + "Gas/data/data_original/"
GAS_DATA_ALL_PATH =                     ISSDA_PATH + "Gas/data/data_all/"
GAS_CUSTOMER_CLASSIFICATION_PATH =      ISSDA_PATH + "Gas/doc/customerClassification.csv"

SCRIPT_RESULTS_PATH =  "./script_results/" 
...
class dataAnalyzer:
    ...
    def writeFilteredData(self, files, firstWeek, lastWeek, type):
        if type == "Electricity":
            data_all_filtered = ELECTRICITY_DATA_ALL_FILTERED_PATH
        else:
            data_all_filtered = GAS_DATA_ALL_FILTERED_PATH
    ...
    def filterDataSetSecondRound(self, data_all_filtered, type, firstWeek, lastWeek):
        ...
        '''
        for file in files:
            print("Removing uncomplete meterIDs from file", file)
            df = pd.read_csv(file)
            df = df[~df.ID.isin(uncompleted_meter_ids)]
            df.to_csv(file, index=False)

        if type == "Electricity":
            customerClassification = ELECTRICITY_CUSTOMER_CLASSIFICATION_PATH
        else:
            customerClassification = GAS_CUSTOMER_CLASSIFICATION_PATH
    ...
    def generateSortedFiles(self, files):
        for file in files:
            ...
            sortedfile = ELECTRICITY_DATA_ORIGINAL_SORTED_PATH + file[-9:]
    ...
    def generateWeekDataFiles(self, files):
        ...
        weekfiles = []
        iweek = []
        path = ELECTRICITY_DATA_ALL_PATH + "ElectricityDataWeek "
...
def electricity_preprocessing():
    ...
    data_original_dir = ELECTRICITY_DATA_ORIGINAL_PATH
    data_all = ELECTRICITY_DATA_ALL_PATH
    data_all_filtered = ELECTRICITY_DATA_ALL_FILTERED_PATH
    ...
    results_file = SCRIPT_RESULTS_PATH + "meterID_electricity.csv"
    mg.meterID.to_csv(results_file, index=False)
...
def gas_preprocessing():
    ...
    # data_original -> data_all
    data_original_dir = GAS_DATA_ORIGINAL_PATH
    data_all = GAS_DATA_ALL_PATH
    data_all_filtered = GAS_DATA_ALL_FILTERED_PATH
    ...
    results_file = SCRIPT_RESULTS_PATH + "meterID_gas.csv"
    mg.meterID.to_csv(results_file, index=False)
...
def check_preconditions():
    ...
    if len(sys.argv) != 2 or (sys.argv[1] != "Electricity" and sys.argv[1] != "Gas"):
        print("Usage (from the diaspore directory): python3 issdacer.py <Electricity/Gas>")
        exit(85)

    # Preventing execution error (Electricity)
    electricity_data_directories_not_exists = not os.path.isdir(ELECTRICITY_DATA_ORIGINAL_PATH) or \
                                         not os.path.isdir(ELECTRICITY_DATA_ALL_PATH) or \
                                         not os.path.isdir(ELECTRICITY_DATA_ALL_FILTERED_PATH) or \
                                         not os.path.exists(ELECTRICITY_CUSTOMER_CLASSIFICATION_PATH)


    if electricity_data_directories_not_exists:
        print("The following directories are needed:")
        print(ELECTRICITY_DATA_ORIGINAL_PATH)
        print(ELECTRICITY_DATA_ALL_PATH)
        print(ELECTRICITY_DATA_ALL_FILTERED_PATH)
        print(ELECTRICITY_CUSTOMER_CLASSIFICATION_PATH)
        exit(1)

    # Preventing execution error (Gas)
    gas_data_directories_not_exists = not os.path.isdir(GAS_DATA_ORIGINAL_PATH) or \
                                      not os.path.isdir(GAS_DATA_ALL_PATH) or \
                                      not os.path.isdir(GAS_DATA_ALL_FILTERED_PATH) or \
                                      not os.path.exists(GAS_CUSTOMER_CLASSIFICATION_PATH)

    if gas_data_directories_not_exists:
        print("The following directories are needed:")
        print(GAS_DATA_ORIGINAL_PATH)
        print(GAS_DATA_ALL_PATH)
        print(GAS_DATA_ALL_FILTERED_PATH)
        print(GAS_CUSTOMER_CLASSIFICATION_PATH)
        exit(1)

    # Preventing bad behaviour with appending mode
    electricity_data_all_is_not_empty = len(os.listdir(ELECTRICITY_DATA_ALL_PATH)) > 0
    if electricity_data_all_is_not_empty and sys.argv[1] == "Electricity":
        print("Remove all files of " + ELECTRICITY_DATA_ALL_PATH + " before executing this script")
        exit(1)
